...
function App() {
  ...
  // ===== Undo helpers =====
  function rebuildFromMoveLog(moves) {
    let b = initialBoard();
    let caps = { red: [], black: [] };
    let lastFrom = null, lastTo = null, capturedAtLocal = null;
    let turnNext = "red";
    let rep = {};
    rep[boardKey(b, turnNext)] = 1;

    for (let i = 0; i < moves.length; i++) {
      const m = moves[i];
      const target = b[m.to[0]][m.to[1]];
      if (target) {
        caps[target.c] = [...caps[target.c], target];
      }
      b = applyMove(b, m.from, m.to);
      lastFrom = m.from; lastTo = m.to;
      capturedAtLocal = target ? m.to : null;
      turnNext = (turnNext === "red") ? "black" : "red";
      const key = boardKey(b, turnNext);
      rep[key] = (rep[key] || 0) + 1;
    }

    const ck = isInCheck(b, turnNext) ? findKing(b, turnNext) : null;
    return { board: b, capturedPieces: caps, lastFrom, lastTo, capturedAt: capturedAtLocal, turnNext, repetitionMap: rep, checkKing: ck };
  }

  // Undo MY last move (red). If AI just moved (turn === 'red'), undo two plies.
  // If I just moved (turn === 'black'), undo one ply.
  function handleUndo() {
    if (aiThinking || moveLog.length === 0) return;

    let pliesToUndo = 1;
    if (turn === 'red') {
      // It's my turn now => AI just moved. Remove AI move + my previous move.
      pliesToUndo = Math.min(2, moveLog.length);
    }
    const trimmed = moveLog.slice(0, moveLog.length - pliesToUndo);
    const st = rebuildFromMoveLog(trimmed);

    setBoard(st.board);
    setCapturedPieces(st.capturedPieces);
    setLastMoveFrom(st.lastFrom);
    setLastMoveTo(st.lastTo);
    setCapturedAt(st.capturedAt);
    setTurn(st.turnNext);
    setRepetitionMap(st.repetitionMap);
    setCheckKing(st.checkKing);
  ...
  .map((reversedIndex) => {
    ...
    return (
      <tr key={reversedIndex}>
        ...
        {/* Buttons row: Undo (left) + Reset (right) */}
        <Box sx={{ mt: 'auto', display: 'flex', gap: 1, width: '100%' }}>
          <Button
            variant="outlined"
            onClick={handleUndo}
            disabled={moveLog.length === 0 || aiThinking}
            sx={{ width: '50%' }}
          >
            Undo
          </Button>
